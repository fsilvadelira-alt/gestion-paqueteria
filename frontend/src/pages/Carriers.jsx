...
const Carriers = () => {
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        if (editId) await api.put(`/carriers/${editId}`, formData);
    ...
    return (
        <div>
            <div className="title-row">
                <h2>Proveedores</h2>
                <button className="btn btn-primary" onClick={() => openForm()}><Plus size={18} /> Nuevo Proveedor</button>
            </div>
                ...
                <div className="modal-overlay">
                    <div className="modal-content">
                        <div className="modal-header">
                            <h3>{editId ? 'Editar' : 'Nuevo'} Proveedor</h3>
                            <button className="modal-close" onClick={() => setShowModal(false)}>Cerrar</button>
                        </div>
                        <div className="modal-body">
                            <form onSubmit={handleSubmit}>
                                <div className="form-group"><label>Nombre del proveedor</label><input required className="form-control" type="text" value={formData.name} onChange={e => setFormData({ ...formData, name: e.target.value })} /></div>
